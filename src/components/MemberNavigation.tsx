...
export default function MemberNavigation({ currentPath = '', tenantId }: MemberNavigationProps) {
  ...
  const memberMenuItems = [
    {
      ...
      href: `/member/agenda`,
      icon: '📅',
      description: 'Mi horario y disponibilidad'
    },
    {
      name: 'Mis Citas',
      href: `/member/appointments`,
      icon: '📋',
      description: 'Citas que debo atender'
    },
    {
      name: 'Mis Pacientes',
            ...
            <div className="relative">
              ...
              >
                <div className="h-8 w-8 rounded-full bg-indigo-500 flex items-center justify-center text-white font-medium">
                  {user?.profile?.first_name?.charAt(0) || user?.email?.charAt(0)?.toUpperCase() || 'M'}
                </div>
                        ...
                        {memberMenuItems.map((item) => (
                          <Link
                            ...
                            className={`block px-4 py-2 text-sm hover:bg-gray-50 transition-colors ${
                              currentPath === item.href ? 'bg-indigo-50 text-indigo-600 border-r-2 border-indigo-600' : 'text-gray-700'
                            }`}
                          >
                            <div className="flex items-center">
